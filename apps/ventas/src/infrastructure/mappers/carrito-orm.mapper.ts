...
export class CarritoOrmMapper {
  static toDomain(entity: CarritoOrmEntity): Carrito {
    ...
    if (entity.items) {
      entity.items.forEach((item) => {
        ...
        itemsMap.set(
          itemDomain.getProductoRef().getProductoId().getValue(),
          itemDomain,
        );
      });
    }
      ...
      entity.descuentos.forEach((d: any) => {
        descuentos.push(
          Descuento.reconstruct(
            ...
            d.montoDescontado
              ? Money.crear(
                  d.montoDescontado.cantidadDecimal,
                  d.montoDescontado.codigoMoneda as string,
                )
              : Money.cero(),
          ),
        );
  ...
  static toPersistance(carrito: Carrito): CarritoOrmEntity {
    ...
    entity.items = carrito
      .getItems()
      .map((item) => ItemCarritoOrmMapper.toPersistance(item));
