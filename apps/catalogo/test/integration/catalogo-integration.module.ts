...
/**
 * Misma composición que CatalogoModule pero sin RabbitMQ: permite pruebas de
 * integración locales (SQLite en memoria + EventEmitter) sin broker.
 */
@Module({
  imports: [
      ...
      dataSourceFactory: async (options?: DataSourceOptions) => {
        if (!options) {
          throw new DomainException(
            'DataSourceOptions no es compatible en este contexto',
          );
        }
  ...
  exports: [ProductoService, ProductoEstadisticasService],
})
export class CatalogoIntegrationTestModule {}
