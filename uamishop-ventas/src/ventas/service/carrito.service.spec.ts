...
describe('CarritoService', () => {
  ...
  describe('obtenerCarrito', () => {
    it('debe obtener un carrito existente', async () => {
      ...
      expect(encontrado.id).toBe(creado.getId().getValue());
    });

    it('debe lanzar EntityNotFoundException si el carrito no existe', async () => {
      const carritoId = CarritoId.generar();

      await expect(service.obtenerCarrito(carritoId)).rejects.toThrow(
  ...
  describe('gestion de productos', () => {
    ...
    it('debe agregar un producto al carrito', async () => {
      const carrito = await service.agregarProducto(
        carritoId,
        productoRef,
        2,
        precio,
      ...
      expect(carrito.getItems()[0].toPrimitives().productoRef.productoId).toBe(
        productoId.getValue(),
      );
